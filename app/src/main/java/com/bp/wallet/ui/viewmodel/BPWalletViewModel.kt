package com.bp.wallet.ui.viewmodel
...
class BPWalletViewModel : ViewModel() {
    ...
    private val _uiEvent = kotlinx.coroutines.channels.Channel<com.bp.wallet.util.UiEvent>()
    val uiEvent = _uiEvent.receiveAsFlow()

    fun sendUiEvent(event: com.bp.wallet.util.UiEvent) {
        viewModelScope.launch {
            _uiEvent.send(event)
        }
    }
    ...
    fun showSnack(message: String) {
        ...
        sendUiEvent(com.bp.wallet.util.UiEvent.ShowSnackbar(message))
    }

    fun showToast(message: String, isLong: Boolean = false) {
        sendUiEvent(com.bp.wallet.util.UiEvent.ShowToast(message, isLong))
    }
    ...
    private fun <T> handleResult(
        ...
        result.onSuccess {
            ...
            val message = com.bp.wallet.util.ErrorHandler.getErrorMessage(err)
            showSnack(message)
            onError?.invoke(err)
        }
    }
    ...
    fun loginUser(username: String, pass: String) {
        ...
        viewModelScope.launch {
            ...
            result.onSuccess { user ->
                ...
                if (err.message == "PENDING_VERIFICATION") {
                    var userEmail = BPWalletRepository.usersList.value.find { it.username.equals(username, true) }?.email ?: ""
                    if (userEmail.isBlank() && username.contains("@")) userEmail = username
                    _pendingVerificationEmail.value = userEmail
                    showSnack("Email pending verification. A confirmation link has been sent to $userEmail")
                    setScreen(ScreenType.VERIFY_EMAIL)
                    return@onFailure
                }
                val message = com.bp.wallet.util.ErrorHandler.getErrorMessage(err)
    ...
    fun registerUser(
        ...
        viewModelScope.launch {
            ...
            handleResult<UserAccount>(result, onSuccess = { user ->
                ...
                if (user.isVerified) {
                    showSnack("Account registered and verified successfully!")
                    setScreen(if (user.isAdminRole) ScreenType.ADMIN_DASHBOARD else ScreenType.USER_HOME)
                } else {
                    showSnack("Confirmation link sent! Please check your email inbox.")
                    setScreen(ScreenType.VERIFY_EMAIL)
                }
            }, onError = {
                triggerErrorShake()
    ...
    fun verifyEmailWithOtp(email: String, otpCode: String) {
        val trimmedEmail = email.trim()
        val trimmedCode = otpCode.trim()
        if (trimmedEmail.isBlank() || trimmedCode.isBlank()) {
            showSnack("Please enter your email and verification code")
            triggerErrorShake()
            return
        }
        viewModelScope.launch {
            BPWalletRepository.setLoading(true)
            val result = BPWalletRepository.verifyEmailWithOtp(trimmedEmail, trimmedCode)
            BPWalletRepository.setLoading(false)
            result.onSuccess { user ->
                _pendingVerificationEmail.value = ""
                recordUserActivity()
                showSnack("Account verified successfully! Welcome ${user.fullName}")
                setScreen(if (user.isAdminRole) ScreenType.ADMIN_DASHBOARD else ScreenType.USER_HOME)
            }.onFailure { err ->
                showSnack(err.message ?: "Verification failed. Please check your code or tap Instant Activate.")
                triggerErrorShake()
            }
        }
    }

    fun instantActivateUser(emailOrUsername: String) {
        val trimmed = emailOrUsername.trim()
        if (trimmed.isBlank()) {
            showSnack("Please enter your email or username")
            triggerErrorShake()
            return
        }
        viewModelScope.launch {
            BPWalletRepository.setLoading(true)
            val result = BPWalletRepository.instantActivateUser(trimmed)
            BPWalletRepository.setLoading(false)
            result.onSuccess { user ->
                _pendingVerificationEmail.value = ""
                recordUserActivity()
                showSnack("Account activated successfully! Welcome to BP Wallet, ${user.fullName}")
                setScreen(if (user.isAdminRole) ScreenType.ADMIN_DASHBOARD else ScreenType.USER_HOME)
            }.onFailure { err ->
                showSnack(err.message ?: "Activation failed")
                triggerErrorShake()
            }
        }
    }

    fun checkAndVerifyUser(email: String, pass: String) {
        val trimmedEmail = email.trim()
        if (trimmedEmail.isBlank()) {
            showSnack("Please enter your email address")
            return
        }
        viewModelScope.launch {
            BPWalletRepository.setLoading(true)
            val result = BPWalletRepository.checkAndVerifyUser(trimmedEmail, pass)
            BPWalletRepository.setLoading(false)
            result.onSuccess { user ->
                _pendingVerificationEmail.value = ""
                recordUserActivity()
                ...
                showSnack(err.message ?: "Verification check failed. Please check your email.")
                triggerErrorShake()
            }
        }
    }
    ...
    fun resendVerification(email: String) {
        ...
        viewModelScope.launch {
            BPWalletRepository.setLoading(true)
            val result = BPWalletRepository.resendVerification(trimmed)
            BPWalletRepository.setLoading(false)
            handleResult(result, onSuccess = {
                showSnack("Confirmation link re-sent! Please check your email inbox.")
            }, onError = {
                triggerErrorShake()
            })
        }
    ...
    fun addPaymentGateway(
        ...
        val res = BPWalletRepository.addPaymentGateway(gw)
        BPWalletRepository.setLoading(false)
        res.onSuccess {
    ...
    fun updatePaymentGateway(
        ...
        val res = BPWalletRepository.updatePaymentGateway(gw)
        BPWalletRepository.setLoading(false)
        res.onSuccess {
    ...
    fun sendAdminNotification(title: String, message: String, type: String) {
        ...
        val notification = com.bp.wallet.model.AdminNotification(
            title = title,
            message = message,
            type = type
        )
    ...
    fun saveAppSettings(settings: com.bp.wallet.model.AppSettings) {
